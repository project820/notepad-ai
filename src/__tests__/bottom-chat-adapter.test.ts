/**
 * bottom-chat-adapter.test.ts
 *
 * Unit tests for `buildBottomChatInstructions(req, assemble?)`.
 *
 * Sub-AC 6.3c requirements:
 *   ✓ The Bottom Chat surface adapter does not throw when the injected assembler
 *     returns a fallback-only stack.
 *   ✓ The Bottom Chat surface adapter returns a non-empty string when the injected
...
import {
  buildBottomChatInstructions,
  BOTTOM_CHAT_SURFACE_PROMPT,
  type BottomChatPromptRequest,
  type AssemblerFn,
} from '../renderer/bottom-chat-adapter';
...
const TOGGLE_OFF_MINIMAL: BottomChatPromptRequest = {
  toggleEnabled: false,
};

/** Toggle-off request with quality directive and document text. */
const TOGGLE_OFF_FULL: BottomChatPromptRequest = {
  toggleEnabled: false,
  qualityDirectiveStr: 'Write at a professional level.',
  documentText: '# My Document\nSome existing content.',
};

/** Fully-populated toggle-on request. */
const TOGGLE_ON_FULL: BottomChatPromptRequest = {
  toggleEnabled: true,
...
const TOGGLE_ON_MINIMAL: BottomChatPromptRequest = {
  toggleEnabled: true,
};
...
describe('A: no-throw guarantee — adapter never throws', () => {
  it('A1. [toggle-ON, fallback assembler] does not throw with minimal params', () => {
    expect(() => buildBottomChatInstructions(TOGGLE_ON_MINIMAL, mockAssembler)).not.toThrow();
  });

  it('A2. [toggle-ON, fallback assembler] does not throw with full params', () => {
    expect(() => buildBottomChatInstructions(TOGGLE_ON_FULL, mockAssembler)).not.toThrow();
  });

  it('A3. [toggle-OFF] does not throw with minimal params', () => {
    expect(() => buildBottomChatInstructions(TOGGLE_OFF_MINIMAL, mockAssembler)).not.toThrow();
  });

  it('A4. [toggle-OFF] does not throw with full params', () => {
    expect(() => buildBottomChatInstructions(TOGGLE_OFF_FULL, mockAssembler)).not.toThrow();
  });
  ...
  it('A5. [toggle-ON] does not throw when assembler returns empty string', () => {
    ...
    expect(() => buildBottomChatInstructions(TOGGLE_ON_MINIMAL, emptyAssembler)).not.toThrow();
  });

  it('A6. [toggle-ON] does not throw when all optional params are explicitly empty strings', () => {
    const emptyParams: BottomChatPromptRequest = {
      toggleEnabled: true,
      systemlawContent: '',
      ownerContent: '',
      qualityDirectiveStr: '',
    ...
    expect(() => buildBottomChatInstructions(emptyParams, mockAssembler)).not.toThrow();
  });

  it('A7. [toggle-ON] does not throw when optional params are undefined', () => {
    const params: BottomChatPromptRequest = {
      toggleEnabled: true,
    ...
    expect(() => buildBottomChatInstructions(params, mockAssembler)).not.toThrow();
  });
});

// ===========================================================================
// B. fallback-only stack — PRIMARY AC requirement (Sub-AC 6.3c)
// ===========================================================================

describe('B: fallback-only stack — primary Sub-AC 6.3c requirement', () => {
  /**
   * PRIMARY TEST for Sub-AC 6.3c:
   * When the assembler is mocked to return a fallback-only stack,
   * the Bottom Chat adapter must (1) not throw, AND (2) return a non-empty string.
   */
  it('B1. [PRIMARY] toggle-ON with fallback assembler: does not throw AND returns non-empty string', () => {
    ...
    expect(() => {
      result = buildBottomChatInstructions(TOGGLE_ON_MINIMAL, mockAssembler);
    }).not.toThrow();
  ...
  it('B2. result equals what the fallback assembler returned (no transformation)', () => {
    const result = buildBottomChatInstructions(TOGGLE_ON_MINIMAL, mockAssembler);
  ...
  it('B4. fallback-only stack with full params: does not throw AND returns non-empty string', () => {
    let result: string | undefined;
    expect(() => {
      result = buildBottomChatInstructions(TOGGLE_ON_FULL, mockAssembler);
    }).not.toThrow();
    expect(typeof result).toBe('string');
    expect((result as string).length).toBeGreaterThan(0);
  });

  it('B5. assembler is called exactly once per buildBottomChatInstructions call (toggle-ON)', () => {
    buildBottomChatInstructions(TOGGLE_ON_MINIMAL, mockAssembler);
    expect(mockAssembler).toHaveBeenCalledTimes(1);
  });

  it('B6. assembler is called with surface: "BottomChat" (toggle-ON)', () => {
    buildBottomChatInstructions(TOGGLE_ON_MINIMAL, mockAssembler);
    expect(mockAssembler).toHaveBeenCalledWith(
      expect.objectContaining({ surface: 'BottomChat' }),
    );
  });
...
describe('C: toggle-OFF path — v1.0 legacy string, assembler not called', () => {
  it('C1. [toggle-OFF] returns a non-empty string without calling the assembler', () => {
    const result = buildBottomChatInstructions(TOGGLE_OFF_MINIMAL, mockAssembler);
    expect(typeof result).toBe('string');
    expect(result.length).toBeGreaterThan(0);
    expect(mockAssembler).not.toHaveBeenCalled();
  });

  it('C2. [toggle-OFF] result contains BOTTOM_CHAT_SURFACE_PROMPT verbatim', () => {
    const result = buildBottomChatInstructions(TOGGLE_OFF_MINIMAL, mockAssembler);
    expect(result).toContain(BOTTOM_CHAT_SURFACE_PROMPT);
  });

  it('C3. [toggle-OFF] quality directive is appended when non-empty', () => {
    const q = 'Write at a professional level.';
    const result = buildBottomChatInstructions({ toggleEnabled: false, qualityDirectiveStr: q }, mockAssembler);
  ...
  it('C5. [toggle-OFF] document content appears between === markers when provided', () => {
    ...
    const result = buildBottomChatInstructions({ toggleEnabled: false, documentText: doc }, mockAssembler);
    expect(result).toContain(`=== Current document ===\n${doc}\n=== End document ===`);
  });

  it('C6. [toggle-OFF] "(empty)" appears when documentText is an empty string', () => {
    const result = buildBottomChatInstructions({ toggleEnabled: false, documentText: '' }, mockAssembler);
  ...
  it('C7. [toggle-OFF] systemlawContent and ownerContent are ignored', () => {
    ...
    const result = buildBottomChatInstructions(params, mockAssembler);
  ...
  it('C8. [toggle-OFF] assembler is NEVER called regardless of other fields', () => {
    buildBottomChatInstructions(TOGGLE_OFF_FULL, mockAssembler);
    expect(mockAssembler).not.toHaveBeenCalled();
  });
});
...
describe('D: toggle-ON path — assembler receives correct AssemblyRequest', () => {
  it('D1. assembler receives systemlawContent from params', () => {
    buildBottomChatInstructions({ toggleEnabled: true, systemlawContent: 'MY_SYSTEMLAW' }, mockAssembler);
    expect(mockAssembler).toHaveBeenCalledWith(
  ...
  it('D2. assembler receives ownerContent from params', () => {
    buildBottomChatInstructions({ toggleEnabled: true, ownerContent: 'MY_OWNER' }, mockAssembler);
    expect(mockAssembler).toHaveBeenCalledWith(
  ...
  it('D3. assembler receives qualityDirective from qualityDirectiveStr param', () => {
    buildBottomChatInstructions({ toggleEnabled: true, qualityDirectiveStr: 'QUALITY_DIRECTIVE' }, mockAssembler);
    expect(mockAssembler).toHaveBeenCalledWith(
  ...
  it('D4. assembler receives documentText from params', () => {
    buildBottomChatInstructions({ toggleEnabled: true, documentText: 'DOCUMENT_TEXT' }, mockAssembler);
    expect(mockAssembler).toHaveBeenCalledWith(
      expect.objectContaining({ documentText: 'DOCUMENT_TEXT' }),
    );
  });

  it('D5. assembler receives surfacePrompt = BOTTOM_CHAT_SURFACE_PROMPT (Layer 3)', () => {
    buildBottomChatInstructions(TOGGLE_ON_MINIMAL, mockAssembler);
    expect(mockAssembler).toHaveBeenCalledWith(
      expect.objectContaining({ surfacePrompt: BOTTOM_CHAT_SURFACE_PROMPT }),
    );
  });
  ...
  it('D6. absent params default to empty string in AssemblyRequest', () => {
    ...
    buildBottomChatInstructions(TOGGLE_ON_MINIMAL, capturingAssembler);
  ...
  it('D7. assembler is called exactly once per invocation (toggle-ON)', () => {
    buildBottomChatInstructions(TOGGLE_ON_FULL, mockAssembler);
    expect(mockAssembler).toHaveBeenCalledTimes(1);
  });
...
describe('E: empty / partial params — graceful fallback', () => {
  it('E1. [toggle-ON] all-empty params: does not throw, returns string', () => {
    let result: unknown;
    expect(() => {
      result = buildBottomChatInstructions({ toggleEnabled: true }, mockAssembler);
    }).not.toThrow();
    expect(typeof result).toBe('string');
  });
  ...
  it('E2. [toggle-ON] only documentText provided: does not throw, returns string', () => {
    let result: unknown;
    expect(() => {
      result = buildBottomChatInstructions({ toggleEnabled: true, documentText: 'text' }, mockAssembler);
    }).not.toThrow();
    expect(typeof result).toBe('string');
  });
  ...
  it('E3. [toggle-ON] only qualityDirectiveStr provided: does not throw, returns string', () => {
    let result: unknown;
    expect(() => {
      result = buildBottomChatInstructions({ toggleEnabled: true, qualityDirectiveStr: 'quality' }, mockAssembler);
    }).not.toThrow();
    expect(typeof result).toBe('string');
  });

  it('E4. [toggle-OFF] missing qualityDirectiveStr: result contains surface prompt', () => {
    const result = buildBottomChatInstructions({ toggleEnabled: false });
    expect(result).toContain(BOTTOM_CHAT_SURFACE_PROMPT);
  });
  ...
  it('E5. [toggle-ON] whitespace-only qualityDirectiveStr is treated as empty', () => {
    ...
    buildBottomChatInstructions({ toggleEnabled: true, qualityDirectiveStr: '   ' }, capturingAssembler);
...
describe('F: assembler injection — mock vs default', () => {
  it('F1. mock assembler is called instead of the real assemblePrompt (toggle-ON)', () => {
    ...
    const result = buildBottomChatInstructions(TOGGLE_ON_MINIMAL, sentinelAssembler);
  ...
  it('F4. toggle flip: assembler called when ON, not called when OFF', () => {
    ...
    buildBottomChatInstructions(TOGGLE_OFF_MINIMAL, mockAssembler);
...
describe('G: return-type guarantees — result is always a string', () => {
  ...
  it('G2. [toggle-OFF, full params] returns string', () => {
    const result = buildBottomChatInstructions(TOGGLE_OFF_FULL, mockAssembler);
    expect(typeof result).toBe('string');
  });
  ...
  it('G4. [toggle-ON, assembler returning ""] returns empty string (never null)', () => {
    const emptyAssembler = vi.fn<[AssemblyRequest], string>().mockReturnValue('');
    const result = buildBottomChatInstructions(TOGGLE_ON_MINIMAL, emptyAssembler);
    expect(typeof result).toBe('string');
    expect(result).not.toBeNull();
    expect(result).not.toBeUndefined();
    expect(result).toBe('');
  ...
  it('G5. [toggle-OFF] result is always non-empty (surface prompt is always included)', () => {
    const result = buildBottomChatInstructions(TOGGLE_OFF_MINIMAL);
    expect(result.trim().length).toBeGreaterThan(0);
  });
});
...
describe('H: legacy path document section formatting', () => {
  ...
  it('H2. [toggle-OFF] "(empty)" placeholder used when documentText is undefined', () => {
    const result = buildBottomChatInstructions({ toggleEnabled: false }, mockAssembler);
    expect(result).toContain('=== Current document ===\n(empty)\n=== End document ===');
  });
  ...
  it('H7. [toggle-OFF] quality and document section both appear with correct ordering', () => {
    ...
    const result = buildBottomChatInstructions({ toggleEnabled: false, qualityDirectiveStr: q, documentText: doc }, mockAssembler);
