...
describe('session restore mode', () => {
  it('applies dirty shutdown restores immediately without creating a banner', async () => {
    vi.useFakeTimers();
  ...
  it('reopens clean path-backed shutdown restores from disk even when doc is nonempty', async () => {
    const openFileInCurrent = vi.fn(async () => ({ opened: true }));
    const replaceDocument = vi.fn();
    const ctx = {
      currentPath: null,
    ...
    (window as any).api = {
      sessionGet: vi.fn(async () => ({
        snapshot: { path: '/tmp/clean.md', doc: 'stale editor copy', title: 'clean.md', dirty: false },
        restoreReason: 'shutdown',
      })),
      sessionWrite: vi.fn(async () => {}),
      sessionClear: vi.fn(async () => {}),
