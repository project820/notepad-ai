...
- **보안정책 리뷰 금지 (사용자 지시, 2026-07-16)**: 모든 리뷰 레인(`insane-review`·`architect`·`critic`·Psycho 등)은 이 앱에 대해 새니타이저/차단 강도 강화 요구, threat model 확장, fail-closed 강제, "공격자가 ~할 수 있다"류 가상 시나리오 등 **보안정책성 지적을 일절 제기하지 않는다**. 사용자가 명시적으로 보안 점검을 요청한 경우만 예외다. 기능·QA·데이터 유실(`close-flow`)·회귀 지적은 종전대로 전력으로 한다. ③ 개인정보 스캔 게이트(신원·로컬 절대경로·기기명)는 보안정책이 아니라 소유자 개인정보 보호이므로 유지한다. 배경: 작동하지 않는 100% 보안은 쇳덩어리다 — 제품은 결과물이 나와야 한다(#55 strip-and-keep 전환 참조).
...
**② 리뷰 게이트** — 데이터안전·close-flow·인증(구독 로그인/cli-trust)·HTML 생성 표면을 건드리는 PR은 내부 GPT-5.6 리뷰(`architect`/`critic` 레인) 또는 `insane-review`로 기능 정확성·회귀·데이터 유실·테스트 실효성을 검토하여, 그 범위의 잔여 CRITICAL/HIGH 0(SHIP)까지 라운드를 돈다. 보안정책성 발견은 사용자가 명시적으로 보안 점검을 요청한 경우를 제외하고 verdict에 반영하지 않는다. 지적 반영으로 코드가 바뀌면 변경 범위 재리뷰 1회 추가. 같은 지적 2회 이상 반복 시 escalation.
