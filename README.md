...
- 🎨 **AI가 문서를 웹페이지로** — 마크다운을 한 번에 스타일 입힌 HTML 문서나 슬라이드로 내보냅니다. 성공적으로 처리한 응답의 위험 요소만 자동 정리하며, 응답이 비어 있거나 해석할 수 없으면 명확한 오류를 표시합니다.
...
## 무엇이 새로워졌나요
- **v0.8.0** — HTML 생성 위저드가 바로 열리고, 자동/디테일 흐름에서 고른 모델을 재시도에도 유지합니다. 자세한 변경 사항은 [CHANGELOG](./CHANGELOG.md)에서 확인하세요.
...
- **AI HTML 내보내기(고도화)** — 문서를 한 번에 스타일 입힌 HTML 페이지나 슬라이드로 변환합니다. **자동/디테일** 모드에서 용도·표현 스타일·방향·밀도·디자인 소스를 고르고, 모델 선택은 재시도에도 유지됩니다. 결과는 외부 의존이 없는 자체 완결형(self-contained)으로 저장·미리보기하며, 성공적으로 처리한 응답의 위험 요소만 자동 정리하고 응답이 비어 있거나 해석할 수 없으면 명확한 오류를 표시합니다.
